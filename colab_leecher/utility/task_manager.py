...
async def taskScheduler():
    ...
    Messages.task_msg = f"<b>🦞 MODO TAREA » </b>"
    Messages.dump_task = (
        Messages.task_msg
    ...
    Messages.download_name = ""
    Messages.task_msg = ""
    Messages.status_head = f"<b>📥 DESCARGANDO » </b>\n"
    ...
    if is_dir:
        ...
        for link in BOT.SOURCE:
            if is_telegram(link):
                ...
                ida = "🧲"
                Messages.caution_msg = "\n\n⚠️<i><b> Los torrents están estrictamente prohibidos en Google Colab</b>. ¡Intenta evitar los imanes!</i>"
...
async def Do_Mirror(source, is_ytdl, is_zip, is_unzip, is_dualzip):
    if not ospath.exists(Paths.MOUNTED_DRIVE):
        await cancelTask(
            "¡Google Drive NO ESTÁ MONTADO! Detenga el robot y ejecute Google Drive Cell para montarlo, luego inténtelo de nuevo."
        )
        return
