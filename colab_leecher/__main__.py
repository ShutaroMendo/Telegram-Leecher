...
async def start(client, message):
    ...
    text = "**Hola, 👋🏼 Soy Colab Leecher**\n\n◲ Soy un potente robot de transferencia de archivos 🚀\n◲ Puedo transferir archivos a Telegram o a Google Drive desde varias fuentes 🦐"
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "Repository 🦄",
                    url="https://github.com/ShutaroMendo/Telegram-Leecher",
                ),
...
async def telegram_upload(client, message):
    ...
    text = "<b>⚡ Envíame ENLACES DE DESCARGA 🔗»</b>\n\n🦀 Sigue el siguiente patrón\n\n<code>https//linktofile1.mp4\nhttps//linktofile2.mp4\n[Espacio de nombres personalizado. mp4]\n{Contraseña para comprimir}\n(Contraseña para descomprimir)</code>"
...
async def yt_upload(client, message):
    ...
    text = "<b>⚡ Enviar ENLACE(s) DE DESCARGA YTDL 🔗»</b>\n\n🦀 Siga el siguiente patrón\n\n<code>https//linktofile1.mp4\nhttps//linktofile2.mp4\n[Espacio de nombres personalizado. mp4]\n{Contraseña para comprimir}</code>"
...
async def handle_url(client, message):
    ...
    if BOT.State.task_going == False and BOT.State.started:
        ...
        await message.reply_text(
            text=f"<b>🐹 Seleccione el tipo de {BOT.Mode.mode.capitalize()} que desea » </b>\n\nRegular:<i> Carga de archivos normal</i>\nComprimir:<i> Carga de archivos comprimidos</i> \nExtract:<i> extraer antes de cargar</i>\nUnDoubleZip:<i> descomprimir y luego comprimir</i>",
        ...
        await message.delete()
        await message.reply_text(
            "<i>¡Ya estoy trabajando! ¡¡Por favor espera hasta que termine 😣!!</i>"
        )
...
async def handle_options(client, callback_query):
    ...
    if callback_query.data in ["normal", "zip", "unzip", "undzip"]:
        ...
        MSG.status_msg = await colab_bot.send_message(
            chat_id=OWNER,
            text="#STARTING_TASK\n\n**Comenzando tu tarea en unos segundos...🦐**",
...
async def help_command(client, message):
    msg = await message.reply_text(
        "Enviar /start para comprobar si estoy vivo 🤨\n\nEnviar /colabxr y seguir las indicaciones para comenzar a transcargar 🚀\n\nEnviar /settings para editar la configuración del bot ⚙️\n\nEnviar /setname para establecer un nombre de archivo personalizado 📛\n\nEnviar /zipaswd para establecer la contraseña para el archivo zip 🔐\n\nEnviar /unzipaswd para establecer la contraseña para extraer Archivos 🔓\n\n⚠️ **SIEMPRE puedes ENVIAR una imagen para configurarla como MINIATURA de tus archivos 🌄**",
        quote=True,
        reply_markup=InlineKeyboardMarkup(
            [
                [
